/**
 * TitanicSimulator v3 — Stateless immersive emergency engine.
 * No instance state. All methods are pure: input state + decision → output state.
 * Safe for Vercel serverless / Edge runtime.
 */
...
const TIMELINE = [
  ...
  },
  {
    ...
    narrative: (s, p) => {
      const temp = (p.Sex === 'female' && p.Age < 16) ? 28 : 31;
...
export class TitanicSimulator {
  ...
  static process(state, decisionId) {
    ...
    // Generate next narrative or final
    let narrative;
    if (state.event_idx < TIMELINE.length) {
